...
### Reshape data
The first step in this analysis is to explor that differnece in element concentrations between furnaces.  To do do, we need to summarise the 105 measurements from each furnace. [note: there are quicker ways to do this, but it demonstrates the point of piping data between data manipulating functions]
```{r reshape, comment=''}
...
The `ggplot` below takes the results of the previous processing, filters to the mean measurment, and plots them over elements by furnace.
...
Described as simply modeling, statistical learning, or machine learning the process of training models via the optimization of hyperparameters for the purpose of prediction is a very powerful approach in modern statistics and data science.  In a somewhat false dichotomy, statistical learning is differeniated from statistical analysis by the goal of the modeling enterprise; prediction vs. inference.  Below is a quick example of using a very popular learning algorithm, extreme gradient boosting, for the classificaiton of Roman glass furnaces and an exploration of which elements differentiate them.  There are a number of clustering methods that could also be used to approach these data.
